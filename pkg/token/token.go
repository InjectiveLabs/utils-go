...
const alchemyEndpointEnvKey = "EXCHANGE_ALCHEMY_ENDPOINT"

var alchemyEndpoint = "https://eth-mainnet.alchemyapi.io/v2/vSqxSDsS7fSB0VNQfWC1r0yVq5QCTy_n"

func readAlchemyEndpointFromEnv() {
	value, ok := os.LookupEnv(alchemyEndpointEnvKey)
	if ok && value != "" {
		log.Infof("got alchemy endpoint from runtime env: [%s]\n", value)
		alchemyEndpoint = value
	}
}

func init() {
	readAlchemyEndpointFromEnv()
	var tokenMap Dict
